...
window.onload = () => {
    ...
    const playMaze = (event) => {
            ...
            if (xPos < rect1.right && xPos > rect1.left && yPos < rect1.bottom && yPos > rect1.top) {
                loseGame();
            }
            if (xPos < rect2.right && xPos > rect2.left && yPos < rect2.bottom && yPos > rect2.top) {
                loseGame();
            }
            if (xPos < rect3.right && xPos > rect3.left && yPos < rect3.bottom && yPos > rect3.top) {
                loseGame();
            }
            if (xPos < rect4.right && xPos > rect4.left && yPos < rect4.bottom && yPos > rect4.top) {
                loseGame();
            }
            if (xPos < rect5.right && xPos > rect5.left && yPos < rect5.bottom && yPos > rect5.top) {
                loseGame();
            }
